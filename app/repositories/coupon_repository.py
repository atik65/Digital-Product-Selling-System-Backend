...
class CouponRepository:
    def get_by_code(self, db: Session, code: str) -> Optional[Coupon]:
        return (
            db.query(Coupon)
            .filter(Coupon.code == code, Coupon.is_deleted.is_(False))
            .first()
        )
    ...
    def get_by_id(self, db: Session, coupon_id: int) -> Optional[Coupon]:
        return (
            db.query(Coupon)
            .filter(Coupon.id == coupon_id, Coupon.is_deleted.is_(False))
            .first()
        )
    ...
    def create(self, db: Session, data: dict) -> Coupon:
        ...
        try:
            ...
            raise DatabaseException(
                f"Failed to create coupon: {str(e)}", original_exception=e
            )
    ...
    def update(self, db: Session, coupon: Coupon, data: dict) -> Coupon:
        try:
            ...
            db.rollback()
            raise DatabaseException(
                f"Failed to update coupon: {str(e)}", original_exception=e
            )
    ...
    def increment_used_count(self, db: Session, coupon_id: int) -> None:
        try:
            ...
            db.rollback()
            raise DatabaseException(
                f"Failed to update coupon count: {str(e)}", original_exception=e
            )
    ...
    def delete(self, db: Session, coupon: Coupon) -> None:
        try:
            ...
            db.rollback()
            raise DatabaseException(
                f"Failed to delete coupon: {str(e)}", original_exception=e
            )
