...
class LotteryService:
    ...
    def check_user_eligibility(
        ...
        if not lottery:
            return LotteryEligibilityResponse(
                ...
                message="No active lottery campaign right now",
            )
        ...
        return LotteryEligibilityResponse(
            ...
            message=f"You have {remaining} spin attempt(s) remaining."
            if remaining > 0
            else "Complete an order to earn lottery spin attempts!",
        )

    def spin_lottery(
        self, db: Session, user_id: int, lottery_id: int
    ) -> LotterySpinResponse:
        ...
        if paid_orders <= used_spins:
            raise ValidationException(
                "You have no remaining lottery attempts. Complete an order to earn spins!"
            )

        # Available prizes with stock
        available_prizes: List[LotteryPrize] = [
            p for p in lottery.prizes if p.quantity > 0 and p.probability > 0
        ]
        ...
        # Record entry
        self.lottery_repo.create_entry(
            db, lottery.id, user_id, selected_prize.id if selected_prize else None
        )
    ...
    def add_prize_to_lottery(
        self, db: Session, lottery_id: int, data: LotteryPrizeCreate
    ) -> LotteryPrize:
        lottery = self.lottery_repo.get_by_id(db, lottery_id)
        if not lottery:
