def test_topup_submission_and_admin_approval(client, admin_auth_headers, auth_headers):
    ...
    init_w = client.get("/wallet/me", headers=auth_headers).json()["data"]["balance"]
    ...
    my_topups = client.get("/wallet/topups/me", headers=auth_headers)
    assert my_topups.status_code == 200
    assert any(t["id"] == topup_id for t in my_topups.json()["data"])

    # 4. Admin lists topups
    admin_topups = client.get("/admin/topups", headers=admin_auth_headers)
    ...
    appr_res = client.post(
        f"/admin/topups/{topup_id}/approve",
    ...
    new_w = client.get("/wallet/me", headers=auth_headers).json()["data"]["balance"]
    assert new_w == init_w + 500.0

    # 7. Check wallet transaction ledger
    txs = client.get("/wallet/transactions", headers=auth_headers).json()["data"]["items"]
...
def test_topup_rejection(client, admin_auth_headers, auth_headers):
    ...
    # Admin rejects
    rej_res = client.post(
        f"/admin/topups/{topup_id}/reject",
    ...
    after_w = client.get("/wallet/me", headers=auth_headers).json()["data"]["balance"]
