def test_wallet_balance_and_pay_order(client, admin_auth_headers, auth_headers, test_user):
    ...
    w_res = client.get("/wallet/me", headers=auth_headers)
    ...
    pay_res = client.post(f"/orders/{order_number}/pay-with-wallet", headers=auth_headers)
    ...
    chk_order = client.get(f"/orders/{order_number}", headers=auth_headers)
    assert chk_order.json()["data"]["status"] == "PAID"

    # 6. Verify ledger transaction recorded
    tx_res = client.get("/wallet/transactions", headers=auth_headers)
...
def test_admin_wallet_adjust(client, admin_auth_headers, auth_headers, test_user):
    ...
    adj_res = client.post(
        f"/admin/wallets/{test_user.id}/adjust",
        json={
            "amount": 500.0,
            "type": "BONUS",
