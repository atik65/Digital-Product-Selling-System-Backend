...
def test_lottery_eligibility_and_spin(client, admin_auth_headers, auth_headers):
    ...
    client.post(
        f"/admin/lotteries/{lottery_id}/prizes",
        json={
    ...
    elig1 = client.get("/lottery/my-eligibility", headers=auth_headers).json()["data"]
    assert elig1["eligible"] is False
    assert elig1["remaining_attempts"] == 0

    # 4. Attempting to spin without eligibility returns 400
    spin_fail = client.post(f"/lottery/{lottery_id}/spin", headers=auth_headers)
    ...
    client.post(f"/orders/{order_num}/pay-with-wallet", headers=auth_headers)

    # 6. Check eligibility now -> 1 attempt available
    elig2 = client.get("/lottery/my-eligibility", headers=auth_headers).json()["data"]
    assert elig2["eligible"] is True
    assert elig2["remaining_attempts"] == 1

    # 7. Customer spins lottery
    spin_res = client.post(f"/lottery/{lottery_id}/spin", headers=auth_headers)
    ...
    # 8. Check my-history
    history = client.get("/lottery/my-history", headers=auth_headers).json()["data"]
    assert len(history) >= 1
    assert history[0]["prize"] is not None
    assert history[0]["prize"]["discount_value"] == 25.0

    # 9. Admin checks entries
    entries = client.get(f"/admin/lotteries/{lottery_id}/entries", headers=admin_auth_headers).json()["data"]
