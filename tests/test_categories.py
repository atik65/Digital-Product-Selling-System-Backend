def test_create_and_list_categories(client, admin_auth_headers):
    ...
    res = client.post("/api/v1/admin/categories", json=create_payload, headers=admin_auth_headers)
    ...
    # Reorder as admin
    res = client.patch(f"/api/v1/admin/categories/{cat_id}/reorder", json={"sort_order": 10}, headers=admin_auth_headers)
    assert res.status_code == 200
